...
def hwf_1d(arr_bool, min_len=3):
    lab, nlab = ndlabel(arr_bool.astype(np.uint8))
    if nlab == 0:
        return np.int16(0)
    counts = np.bincount(lab)[1:]
    valid = counts[counts >= min_len]
    if valid.size == 0:
        return np.int16(0)
    return np.int16(valid.sum())


def HWF_per_year_streaming(hit_bool_da, min_len=3):
    ...
    for i, y in enumerate(tqdm(years, desc="HWF：逐年计算（逐步）", ncols=88)):
        sub = hit_bool_da.sel(time=str(int(y))).chunk({"time": -1})
        hwf_y = xr.apply_ufunc(
            hwf_1d, sub, kwargs={"min_len": min_len},
        ...
        arr[i, :, :] = hwf_y.values
        del sub, hwf_y; gc.collect()
    hwf = xr.DataArray(
        arr,
        coords={"time": years.astype(int), "lat": hit_bool_da.lat, "lon": hit_bool_da.lon},
        dims=("time", "lat", "lon"),
        name="HWF"
    )
    return hwf
...
def load_ca_polygon(shp_dir):
    ...
    ca_poly = unary_union(gdf.geometry)
    ca_boundary = gpd.GeoSeries([ca_poly], crs="EPSG:4326")
    return ca_poly, ca_boundary
...
def mask_outside_polygon(lat, lon, polygon):
    ...
    inside = np.array(
        [polygon.contains(p) or polygon.touches(p)
         for p in tqdm(pts, desc="生成区域掩膜", ncols=88)],
        dtype=bool
    )
...
def plot_diff_single(delta_da, ca_boundary,
    ...
    fig.suptitle("ERA5 Heatwave Frequency (HWF), UTC+05:00", fontsize=12, y=0.97)

    norm = TwoSlopeNorm(vcenter=0.0, vmin=vmin, vmax=vmax)
    pcm = ax.pcolormesh(
        lon, lat, delta_da.values.astype(np.float32),
        cmap="RdBu_r", norm=norm, shading="auto", zorder=1
    )


    ca_boundary.boundary.plot(
        ax=ax, color="k", linewidth=1.0, zorder=3
    )
...
def plot_trend_single(slope_da, p_da, ca_boundary,
    ...
    norm = TwoSlopeNorm(vcenter=0.0, vmin=vmin, vmax=vmax)
    pcm = ax.pcolormesh(
        lon, lat, slope_da.values.astype(np.float32),
        cmap="RdBu_r", norm=norm, shading="auto", zorder=1
    )
    ...
    if sig is not None and np.any(sig):
        ...
        ax.scatter(
            lon2[sig], lat2[sig],
            s=4, c="k", marker=".", linewidths=0,
            alpha=0.7, label=f"MK p < {alpha_sig}", zorder=4
        )
...
def main():
    ...
    print("Step 4/7 逐年 HWF（DL/NL/CL） …")
    print("  -> DL")
    hwf_dl = HWF_per_year_streaming(hit_dl, MIN_HW_LEN); del hit_dl; gc.collect()
    print("  -> NL")
    hwf_nl = HWF_per_year_streaming(hit_nl, MIN_HW_LEN); del hit_nl; gc.collect()
    print("  -> CL")
    hwf_cl = HWF_per_year_streaming(hit_cl, MIN_HW_LEN); del hit_cl; gc.collect()

    print("Step 5/7 计算两阶段年均差（late−early） …")

    def early_late_mean(hwf):
        early = hwf.sel(time=slice(EARLY_START, EARLY_END)).mean("time", skipna=True)
        late  = hwf.sel(time=slice(LATE_START,  LATE_END)).mean("time", skipna=True)
        return early, late

    early_dl, late_dl = early_late_mean(hwf_dl)
    early_nl, late_nl = early_late_mean(hwf_nl)
    early_cl, late_cl = early_late_mean(hwf_cl)
    ...
    slope_dl, p_dl = mk_sen_trend_3d(hwf_dl)
    slope_nl, p_nl = mk_sen_trend_3d(hwf_nl)
    slope_cl, p_cl = mk_sen_trend_3d(hwf_cl)
    ...
    title_dl_diff = "DL (Daytime, TX90p on Tmax)\nΔHWF = Late (1990–2024) − Early (1950–1984)"
    title_dl_trend = "DL (Daytime, TX90p on Tmax)\nTrend of annual HWF (Sen's slope, 1950–2024)"

    plot_diff_single(
        d_dl_m, ca_boundary,
        vmin_diff, vmax_diff,
        title_dl_diff,
        cbar_label="Change in annual HWF (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWF_change_DL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_change_DL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        vmin_trend, vmax_trend,
        title_dl_trend,
        cbar_label="Sen's slope of annual HWF (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWF_trend_DL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_trend_DL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_diff_single(
        ...
        title_nl_diff,
        cbar_label="Change in annual HWF (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWF_change_NL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_change_NL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        cbar_label="Sen's slope of annual HWF (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWF_trend_NL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_trend_NL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_diff_single(
        ...
        title_cl_diff,
        cbar_label="Change in annual HWF (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWF_change_CL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_change_CL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        cbar_label="Sen's slope of annual HWF (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWF_trend_CL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWF_trend_CL_1950-2024_CA_single.pdf"),
    )

    print("[OK] 六张 ERA5 HWF 图已输出到：", OUT_DIR)
