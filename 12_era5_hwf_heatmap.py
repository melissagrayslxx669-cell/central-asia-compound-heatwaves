...
out_matrix = r"G:\CYX\Analyse\ERA5_CA_HWF_matrix.csv"
out_fig = r"G:\CYX\Analyse\figs_ERA5\ERA5_CA_HWF_heatmap.png"
...
def calculate_HWF(daily_df: pd.DataFrame, threshold_period: tuple,
    ...
    monthly_hwf = daily_df[daily_df['heatwave_event'] > 0].resample('ME').size()

    full_index = pd.date_range(start=daily_df.index.min(),
                               end=daily_df.index.max(),
                               freq='ME')
    monthly_counts = pd.Series(0, index=full_index, dtype=int)
    monthly_counts.update(monthly_hwf)
...
def main():
    ...
    print("\n步骤2: 计算HWF...")
    monthly_hwf = calculate_HWF(daily_df, threshold_period,
    ...
    for date, count in monthly_hwf.items():
        year = date.year
        month = date.month
        if year in year_index:
            matrix_data[year_index[year], month-1] = count
    ...
    ax.set_title(
        f"ERA5 Heatwave Frequency (HWF) over Central Asia (1950–2024)\n"
