...
def plot_diff_single(delta_da, ca_boundary,
    ...
    fig.suptitle("ERA5 Heatwave Duration (HWD), UTC+05:00", fontsize=12, y=0.97)
...
def main():
    ...
    print("Step 4/7 逐年 HWD（DL/NL/CL） …")
    print("  -> DL")
    hwd_dl = HWD_per_year_streaming(hit_dl, MIN_HW_LEN); del hit_dl; gc.collect()
    print("  -> NL")
    hwd_nl = HWD_per_year_streaming(hit_nl, MIN_HW_LEN); del hit_nl; gc.collect()
    print("  -> CL")
    hwd_cl = HWD_per_year_streaming(hit_cl, MIN_HW_LEN); del hit_cl; gc.collect()

    print("Step 5/7 计算两阶段年均差（late−early） …")

    def early_late_mean(hwd):
        early = hwd.sel(time=slice(EARLY_START, EARLY_END)).mean("time", skipna=True)
        late  = hwd.sel(time=slice(LATE_START,  LATE_END)).mean("time", skipna=True)
        return early, late

    early_dl, late_dl = early_late_mean(hwd_dl)
    early_nl, late_nl = early_late_mean(hwd_nl)
    early_cl, late_cl = early_late_mean(hwd_cl)
    ...
    slope_dl, p_dl = mk_sen_trend_3d(hwd_dl)
    slope_nl, p_nl = mk_sen_trend_3d(hwd_nl)
    slope_cl, p_cl = mk_sen_trend_3d(hwd_cl)
    ...
    title_dl_diff = "DL (Daytime, TX90p on Tmax)\nΔHWD = Late (1990–2024) − Early (1950–1984)"
    title_dl_trend = "DL (Daytime, TX90p on Tmax)\nTrend of annual HWD (Sen's slope, 1950–2024)"

    plot_diff_single(
        d_dl_m, ca_boundary,
        vmin_diff, vmax_diff,
        title_dl_diff,
        cbar_label="Change in annual mean HWD (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWD_change_DL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_change_DL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        vmin_trend, vmax_trend,
        title_dl_trend,
        cbar_label="Sen's slope of annual HWD (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWD_trend_DL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_trend_DL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_diff_single(
        ...
        title_nl_diff,
        cbar_label="Change in annual mean HWD (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWD_change_NL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_change_NL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        cbar_label="Sen's slope of annual HWD (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWD_trend_NL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_trend_NL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_diff_single(
        ...
        title_cl_diff,
        cbar_label="Change in annual mean HWD (days per year)",
        out_png=os.path.join(OUT_DIR, "era5_HWD_change_CL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_change_CL_1950-2024_CA_single.pdf"),
    )
    ...
    plot_trend_single(
        ...
        cbar_label="Sen's slope of annual HWD (days per year)",
        alpha_sig=0.05,
        out_png=os.path.join(OUT_DIR, "era5_HWD_trend_CL_1950-2024_CA_single.png"),
        out_pdf=os.path.join(OUT_DIR, "era5_HWD_trend_CL_1950-2024_CA_single.pdf"),
    )

    print("[OK] 六张 ERA5 HWD 图已输出到：", OUT_DIR)
